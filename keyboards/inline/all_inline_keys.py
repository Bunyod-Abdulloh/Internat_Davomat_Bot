...
async def generate_multiselect_keyboard(telegram_id: int, next_step: bool = False):
    ...
    for class_ in classes:
        user = await db.select_employee_level(telegram_id=telegram_id, level=class_[0], position='Tarbiyachi')
        if user:
...
async def teachers_multiselect_keyboard(telegram_id: int, next_step: bool = False):
    ...
    classes = await db.select_all_classes()
    for class_ in classes:
        user = await db.select_employee_level(telegram_id=telegram_id, level=class_[0], position='Sinf rahbar')
        if user:
            button_text = f"✅ {class_[0]}"
        else:
            button_text = f"{class_[0]}"
